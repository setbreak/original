...
var fullFeedResults = null;         // The results of the XMLHttpRequest
var bandFeedResults = null;         // The results of the XMLHttpRequest
var yearFeedResults = null;         // The results of the XMLHttpRequest
var showFeedResults = null;         // The results of the XMLHttpRequest
var showFeed = { url: null, title: "", baseURL: null };  // Object to hold information about the current feed

var httpShowFeedRequest = null;     // The current XMLHttpRequest

var songFeed = { url: null, title: "", baseURL: null };  // Object to hold information about the current feed

var httpSongFeedRequest = null;     // The current XMLHttpRequest
var filterSongString = "";          // String to filter results while searching
var songFeedResults = null;         // The results of the XMLHttpRequest
...
// processFeedDocument() will be called when it finishes loading.
//
function fetchSongFeed()
{
    if (!songFeed.url || songFeed.url.length < 1) {
        showError("No RSS song feed specified\n");
        return false;
    }

    // Abort any pending request before starting a new one
    if (httpSongFeedRequest != null) {
        httpSongFeedRequest.abort();
        httpSongFeedRequest = null;
    }
    httpSongFeedRequest = new XMLHttpRequest();

    // Function callback when feed is loaded
    httpSongFeedRequest.onload = function (e)
    {
        var songFeedRootElement;
        if (httpSongFeedRequest.responseXML) songFeedRootElement = httpSongFeedRequest.responseXML.documentElement;

        // Request is no longer pending
        httpSongFeedRequest = null;

        // Process the loaded document
        processSongFeedDocument(songFeedRootElement);
    }

    try{
        httpSongFeedRequest.overrideMimeType("text/xml");
        httpSongFeedRequest.open("GET", songFeed.url);
        httpSongFeedRequest.setRequestHeader("Cache-Control", "no-cache");

        // Send the request asynchronously
        httpSongFeedRequest.send(null);
    } catch (e) {
        if( e == "Error: Permission denied" ){
            showError("Song Feed not available.  The Song feed URL must be from the same source as your web application.");
        }else{
            showError("Song Feed not available.");
        }
        return false;
    }
}
...
// processFeedDocument() will be called when it finishes loading.
//
function fetchShowFeed()
{
    if (!showFeed.url || showFeed.url.length < 1) {
        showError("No RSS show feed specified\n");
        return false;
    }

    // Abort any pending request before starting a new one
    if (httpShowFeedRequest != null) {
        httpShowFeedRequest.abort();
        httpShowFeedRequest = null;
    }
    httpShowFeedRequest = new XMLHttpRequest();

    // Function callback when feed is loaded
    httpShowFeedRequest.onload = function (e)
    {
        var showFeedRootElement;
        if (httpShowFeedRequest.responseXML) showFeedRootElement = httpShowFeedRequest.responseXML.documentElement;

        // Request is no longer pending
        httpShowFeedRequest = null;

        // Process the loaded document
        processShowFeedDocument(showFeedRootElement);
    }

    try{
        httpShowFeedRequest.overrideMimeType("text/xml");
        httpShowFeedRequest.open("GET", songFeed.url);
        httpShowFeedRequest.setRequestHeader("Cache-Control", "no-cache");

        // Send the request asynchronously
        httpShowFeedRequest.send(null);
    } catch (e) {
        if( e == "Error: Permission denied" ){
            showError("Show Feed not available.  The Show feed URL must be from the same source as your web application.");
        }else{
            showError("Show Feed not available.");
        }
        return false;
    }
}
...
function processFeedDocument(doc)
{
     if (doc) {
        ...
        fullFeedResults = results;
//		bandFeedResults = filterEntriesBand(results);
		filterEntriesBandAll(fullFeedResults);
...
// var newIndicatorValue = 10;									// new indicator value
...
// doc: XML document containing the feed
//
function processSongFeedDocument(doc)
{
     if (doc) {
        ...
        var results;
            // It's probably some version of RSS.
            // We don't care as long as it has <item>s
            results = parseSongRSSFeed(doc);

        // Got no results?
        if (results == null || results.length < 1) {
            showError("No Song articles found in feed");
            return;
        }
...
//        }
        ...
        songFeedResults = results;
//        document.getElementById("headlineList").object.reloadData();
//        document.getElementById("secondHeadlines").object.reloadData();
//	document.getElementById("horizontalLevelIndicator").object.setValue(10);
        document.getElementById("songList").object.reloadData();
//    document.getElementById("StackLayout").object.setCurrentView("showPage", false);
...
//        document.getElementById("stackLayout").object.setCurrentView("songProgressLoad", false);
        ...
        showError(songFeed.url + " does not appear to be a valid RSS or Atom feed");
    }
}
...
// doc: XML document containing the feed
//
function processShowFeedDocument(doc)
{
     if (doc) {
        ...
        var results;
            // It's probably some version of RSS.
            // We don't care as long as it has <item>s
            results = parseShowRSSFeed(doc);

        // Got no results?
        if (results == null || results.length < 1) {
            showError("No Show articles found in feed");
            return;
        }

        // Limit entries to top N, search terms, and date
//        results = filterEntries(results);
        results = loadShowDB(results);
...
//        document.getElementById("stackLayout").object.setCurrentView("songProgressLoad", false);
        ...
        showError(showFeed.url + " does not appear to be a valid RSS or Atom feed");
    }
}
...
function parseRSSFeed(rss)
{
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (item.nodeName == "item") {
            ...
            if (title != null) {
                // get the year
                var year = findChild(item, "year");
                 // get the src
                var src = findChild(item, "src");
                // get the venue
                var venue = findChild(item, "venue");
                // get the link
                var link = findChild(item, "link");
                // get the ident
                var ident = findChild(item, "ident");
                // get the source
                var source = findChild(item, "source");
                // get the song
                var song = findChild(item, "song");
                // get the setlist
                var setlist = findChild(item, "setlist");
                // get the lineage
                var lineage = findChild(item, "lineage");
                // get the notes
                var notes = findChild(item, "notes");
                // get the band
                var band = findChild(item, "band");
                // get the band
                var pix = findChild(item, "pix");
                // get the date
                ...
                results[results.length] = {
                    ...
                    song: allData(song),
                    setlist: allData(setlist),
                    lineage: allData(lineage),
                    notes: allData(notes),
                    src: allData(src),
                    band: allData(band),
                    pix: allData(pix),
                    dispband: allData(band),
                    venue: allData(venue),
                    date: new Date(itemDate),
...
function parseSongRSSFeed(rss)
{
    ...
    if (channel) {
        var mainLinkEl = findChild(channel, "link");
        if (mainLinkEl) {
            songFeed.baseURL = splitURL(allData(mainLinkEl));
        }
    }
...
//                var description = findChild(item, "encoded", NS_CONTENT) || findChild(item, "description");
                // get the track
                var track = findChild(item, "track");
                // get the track
                var lyrics = findChild(item, "lyrics");
				// save the result
                results[results.length] = {
                    title: allData(title),
                    link: allData(link),
...
function parseShowRSSFeed(rss)
{
    ...
    if (channel) {
        var mainLinkEl = findChild(channel, "link");
        if (mainLinkEl) {
            showFeed.baseURL = splitURL(allData(mainLinkEl));
        }
    }
...
//    for (var i = entries.length-1; i >= 0; i--) {
		...
		var entryvenueid = entry.venueid;
		var entryvenue = entry.venue;
		var entryaddress = entry.address;
		
		var entrysbd = entry.sbd;
		var entrysource = entry.source;
		var entryidentifier = entry.identifier;
		var entryrecordings = entry.recordings;
    ...
    //    }
        ...
        if (maxAgeToShow == -1 || entryDate >= cutoffDate) {
            ...
            if (filterString==""
                ...
                || (entry.setlist && entry.setlist.match(regExp))
                || (entry.year && entry.year.match(regExp))
                || (entry.source && entry.source.match(regExp))
                || (entry.lineage && entry.lineage.match(regExp))
                || (entry.notes && entry.notes.match(regExp))
                || (entry.src && entry.src.match(regExp))
                || (entry.band && entry.band.match(regExp))
                || (entry.venue && entry.venue.match(regExp))
      ...
      transaction.executeSql('INSERT INTO gdshowsNEWlocal (id,title,description,date,month,day,year,venueid,venue,address,sbd,source,identifier,recordings) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[entryid,entrytitle,entrydescription,entrydate,entrymonth,entryday,entryyear,entryvenueid,entryvenue,entryaddress,entrysbd,entrysource,entryidentifier,entryrecordings],nullDataHandler,defErrorHandler);


      transaction.executeSql('INSERT INTO gdshowsNEWlocal (id,title,description,date,month,day,year,venueid,venue,address,sbd,source,identifier,recordings) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[id,title,description,date,month,day,year,venueid,venue,address,sbd,source,identifier,recordings],nullDataHandler,defErrorHandler);




//				if !((entryBand == lastentryBand) && (entryDate == lastentryDate)) {
  //              result.push(entry);				
	//			}
...
//				if ((entryBand == lastentryBand) && (entryDate != lastentryDate)) {
  //              result.push(entry);				
	//			}
				...
				if (
				((entryBand != lastentryBand) && (entryYear != lastentryYear)) 
				|| ((entryBand == lastentryBand) && (entryYear != lastentryYear)) 
				|| ((entryBand != lastentryBand) && (entryYear == lastentryYear)) 
					){
//                result.push(entry);
...
//				if ( (entryBand != $lastentryBand) || ( (entryBand == $lastentryBand) && (entryDate != lastentryDate) ) ) {
if (1==2) {
				if ((entryBand != lastentryBand) && (entryDate != lastentryDate)) {
                result.push(entry);
				}

					if ((entryBand == lastentryBand) && (entryYear != lastentryYear)) {
					result.push(entry);				
					}
...
//				if ((entryBand != lastentryBand) && (entryDate != lastentryDate)) {
  //              result.push(entry);
	...
	//			}
		...
		lastentryBand = entryBand;
		lastentryYear = entryYear;
...
// Returns array of entries matching the filter(s).
//
function filterEntriesReverse(entries)
{
    var result = new Array();
...
// Returns array of entries matching the filter(s).
//
function filterEntriesBand(entries)
{
    var result = new Array();
    ...
    for (var k = 0; k < result.length; k++) {
			...
			var checkresult = result[k];
			var resultBand = checkresult.band;
        if (entryBand == resultBand) {
            // No date, pretend it's today
            bandAdded = true;
			break;
        }
...
// Returns array of entries matching the filter(s).
//
function filterEntriesBandAll(entries)
{
    var result = new Array();
    var resultBandList = new Array();
    var resultYearList = new Array();
	...
	var bandAdded = false;
	var yearAdded = false;
	result.dispband = "Show All";
	result.band = " ";
	result.title = "all";
			resultBandList.push(result);				
    ...
    for (var k = 0; k < resultBandList.length; k++) {
			
			var checkresult = resultBandList[k];
			var resultBand = checkresult.band;
        if (entryBand == resultBand) {
//			entry.venue++;
            bandAdded = true;
...
//			entry.venue = 1;
//			checkresult.venue = 1;				
			resultBandList.push(entry);				
//			resultBandList[0].venue = 1;
			}
			
    for (var k = 0; k < resultYearList.length; k++) {
			
			var checkresult = resultYearList[k];
			var resultYear = checkresult.year;
        if (entryYear == resultYear) {
            yearAdded = true;
			break;
        }
...
bandFeedResults = resultBandList;
//        bandFeedResults.sort(function (a, b) { return b.band - a.band; });
// bandFeedResults = resultYearList;
yearFeedResults = resultYearList;
...
// url: URL of the feed source
//
function setSongFeedSource(url) {
    if (url) {
        // make sure the url has an appropriate protocol
        url = url.replace(/^(feed:\/\/)/, "http://");
        if (url.indexOf("://") < 0) {
